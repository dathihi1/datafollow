...
def _render_data_model_tab(
    ...
    with col1:
        ...
        with stats_col2:
            st.metric("⏱️ Time Span", f"{len(loads) * 5 / 60 / 24:.1f} days")
        with stats_col3:
            st.metric("📈 Mean Load", f"{np.mean(loads):.1f}")
        with stats_col4:
            st.metric("📊 Peak Load", f"{np.max(loads):.1f}")
...
def _render_simulation_tab(
    ...
    if use_forecast and forecast:
        ...
        st.success(f"Using {len(sim_data):,} forecasted data points")
    else:
        sim_data = loads
        st.info(f"Using {len(sim_data):,} historical data points")
    ...
    if st.button("🚀 Run All Simulations", type="primary", use_container_width=True):
        ...
        st.success("✅ Completed 9 simulations (3 configs × 3 policies)")
    ...
    with col2:
        if st.session_state.recommendation:
            ...
            **Expected Performance:**
            ...
            - 🖥️ Avg Servers: **{rec.avg_servers:.1f}**
            - ⚠️ SLA Violations: **{rec.expected_sla_violations}** ({rec.sla_violation_rate:.2%})
            ...
            with risk_col1:
                risk_color = {"low": "🟢", "medium": "🟡", "high": "🔴"}.get(rec.sla_risk_level, "⚪")
                st.metric("SLA Risk", f"{risk_color} {rec.sla_risk_level.upper()}")
            
            with risk_col2:
                peak_color = {"low": "🟢", "medium": "🟡", "high": "🔴"}.get(rec.peak_hour_risk, "⚪")
